...
A useful picture to keep in mind is <a href="#fig1">Fig. 1</a>. It matches the parent-child recursion used by RNEA. In the serial-chain special case shown here, each joint $i$ connects body $i-1$ to body $i$, so equivalently $\lambda(i)=i-1$ and $\mu(i)=\{i+1\}$. Note that each non-root body may actually have more than one children $\mu(i) = \{ j,\, k,\, l,\, \dots \}$.

<a name="fig1"></a>
![Generic serial kinematic chain used to interpret the RNEA recursions.](./kinematic_chain.png)

*Fig. 1. Kinematic chain <a href="#fs08">[1]</a>.*
...
- $\lambda(i)$: parent index of body $i$; in a kinematic tree each non-root body has exactly one immediate parent
- $\mu(i) = \{ j,\, k,\, l,\, \dots \}$: set of immediate children of body $i$; a body may have zero, one, or multiple immediate children

See <a href="#fig2">Fig. 2</a> for this adjacency convention.
...
$$
\mathcal{V}_i = \text{Ad}_{T_{i,\lambda(i)}} \mathcal{V}_{\lambda(i)} + S_i \dot q_i
$$

Interpretation:

- $T_{i,\lambda(i)}$: the pose of frame $\{\lambda(i)\}$ as expressed in frame $\{i\}$, so $\text{Ad}_{T_{i,\lambda(i)}}$ re-expresses a spatial motion from frame $\{\lambda(i)\}$ coordinates into frame $\{i\}$ coordinates
- $\text{Ad}_{T_{i,\lambda(i)}} \mathcal{V}_{\lambda(i)}$ is the **parent link's twist re-expressed in the child frame**
...
$$
\dot{\mathcal{V}}_i = \text{Ad}_{T_{i,\lambda(i)}} \dot{\mathcal{V}}_{\lambda(i)} + S_i \ddot q_i + \text{ad}_{\mathcal{V}_i}(S_i \dot q_i)
$$

Interpretation:

- $\text{Ad}_{T_{i,\lambda(i)}} \dot{\mathcal{V}}_{\lambda(i)}$: inherited parent acceleration
...
- $\text{Ad}_{T_{i,\lambda(i)}} \dot{\mathcal{V}}_{\lambda(i)}$ is the acceleration body $i$ would have if it were momentarily **welded** to its parent body $\lambda(i)$. It is just the parent's acceleration, re-expressed in frame $i$.
...
$$
\mathcal{F}_i^{\uparrow} = \mathcal{F}_i + \sum_{j \in \mu(i)} \text{Ad}_{T_{i,j}}^* \mathcal{F}_j^{\uparrow}
$$

Here `parent` and `children` mean only the **adjacent** neighbors of body $i$ in the kinematic tree, not all ancestors or all descendants:

- $\lambda(i)$: the immediate parent of body $i$
- $\mu(i) = \{ j,\, k,\, l,\, \dots \}$: the set of immediate children of body $i$

<a name="fig2"></a>

<a href="#fig2">Fig. 2</a> shows exactly this local relationship for one body in a tree:

![Immediate parent and immediate children of body i in the backward wrench recursion.](./force_acting_on_body_i.png)

*Fig. 2. Forces acting on body $i$ <a href="#fs08">[1]</a>.*
...
## 9. Full RNEA algorithm in tree notation

I use the tree notation $\lambda(i)$ and $\mu(i)$ for parent/child relations in the main derivation, while the Modern Robotics comparison in Appendix B specializes this to the serial-chain indexing $i-1$ and $i+1$.
...
We require power to be invariant under coordinate change $T$:
...
So $\text{ad}_{\mathcal{V}} \mathcal{U}$ is the same object written in two equivalent languages: as the **Lie bracket** $[\mathcal{V},\mathcal{U}]$ and as the **6D spatial cross-product** $\mathcal{V} \times \mathcal{U}$.
...
Here the hat map converts the 6D twist vector into its $4 \times 4$ matrix representation in $se(3)$:
...
$$
\exp(\widehat{\mathcal{V}}\, dt) \in SE(3)
$$

is the small rigid transform generated by the twist $\mathcal{V}$ over time $dt$.
